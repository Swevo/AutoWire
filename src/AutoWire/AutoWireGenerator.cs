...
public sealed class AutoWireGenerator : IIncrementalGenerator
{
    ...
    private static readonly DiagnosticDescriptor AW011EmptyInterceptorInterface = new(
        id: "AW011",
        title: "[Interceptor] target interface has no interceptable methods",
        messageFormat: "The interface '{0}' passed to [Interceptor] on '{1}' has no non-generic, non-ref/out instance methods. The generated proxy class will be empty and the interception will have no effect.",
        category: "AutoWire",
        defaultSeverity: DiagnosticSeverity.Warning,
        isEnabledByDefault: true,
        description: "Either pass a concrete interface that declares instance methods, or remove the [Interceptor] attribute. Generic methods and methods with ref/out parameters are not proxied.");

    private static readonly DiagnosticDescriptor AW012DecoratorServiceTypeMismatch = new(
        id: "AW012",
        title: "Decorator does not implement the decorated service type",
        messageFormat: "Decorator '{0}' does not implement '{1}'. Decorators must implement the service they decorate.",
        category: "AutoWire",
        defaultSeverity: DiagnosticSeverity.Error,
        isEnabledByDefault: true,
        description: "Ensure the decorator class implements the service type passed to [DecorateScoped], [DecorateSingleton], or [DecorateTransient].");

    private static readonly DiagnosticDescriptor AW013MissingRegistration = new(
        id: "AW013",
        title: "Constructor parameter is not registered with AutoWire",
        messageFormat: "'{0}' constructor parameter '{1}' is not registered with AutoWire. Ensure it is registered manually or add an AutoWire attribute to '{2}'.",
        category: "AutoWire",
        defaultSeverity: DiagnosticSeverity.Warning,
        isEnabledByDefault: true,
    ...
    private const string AttributeSource = """
        ...
        namespace AutoWire
        {
            ...
            public sealed class ScopedAttribute : Attribute
            {
                ...
                /// <summary>When two or more service types are specified, all types to register against. Takes priority over <see cref="ServiceType"/>.</summary>
                ...
                /// <summary>Registers against multiple explicit service types. Equivalent to stacking multiple [Scoped] attributes.</summary>
                public ScopedAttribute(Type serviceType1, Type serviceType2, params Type[] additionalTypes)
                {
                    ServiceType = serviceType1;
                    ServiceTypes = new[] { serviceType1, serviceType2 }.Concat(additionalTypes).ToArray();
                }
            }
            [AttributeUsage(AttributeTargets.Class, AllowMultiple = true, Inherited = false)]
            public sealed class SingletonAttribute : Attribute
            {
                ...
                /// <summary>Registers against multiple explicit service types. Equivalent to stacking multiple [Singleton] attributes.</summary>
                public SingletonAttribute(Type serviceType1, Type serviceType2, params Type[] additionalTypes)
                {
                    ServiceType = serviceType1;
                    ServiceTypes = new[] { serviceType1, serviceType2 }.Concat(additionalTypes).ToArray();
                }
            ...
            public sealed class TransientAttribute : Attribute
            {
                ...
                /// <summary>Registers against multiple explicit service types. Equivalent to stacking multiple [Transient] attributes.</summary>
                public TransientAttribute(Type serviceType1, Type serviceType2, params Type[] additionalTypes)
                {
                    ServiceType = serviceType1;
                    ServiceTypes = new[] { serviceType1, serviceType2 }.Concat(additionalTypes).ToArray();
                }
            ...
            public sealed class TryScopedAttribute : Attribute
            {
                ...
                public TryScopedAttribute(Type serviceType1, Type serviceType2, params Type[] additionalTypes)
                {
                    ServiceType = serviceType1;
                    ServiceTypes = new[] { serviceType1, serviceType2 }.Concat(additionalTypes).ToArray();
                }
            ...
            public sealed class TrySingletonAttribute : Attribute
            {
                ...
                public TrySingletonAttribute(Type serviceType1, Type serviceType2, params Type[] additionalTypes)
                {
                    ServiceType = serviceType1;
                    ServiceTypes = new[] { serviceType1, serviceType2 }.Concat(additionalTypes).ToArray();
                }
            ...
            public sealed class TryTransientAttribute : Attribute
            {
                ...
                public TryTransientAttribute(Type serviceType1, Type serviceType2, params Type[] additionalTypes)
                {
                    ServiceType = serviceType1;
                    ServiceTypes = new[] { serviceType1, serviceType2 }.Concat(additionalTypes).ToArray();
                }
            ...
            public sealed class HttpClientAttribute : Attribute
            {
                ...
                /// <summary>
                /// When true, registers the named client via <c>IHttpClientFactory</c> instead of a typed client.
                /// Emits <c>services.AddHttpClient(Name)</c> and a scoped factory delegate
                /// <c>sp => sp.GetRequiredService&lt;IHttpClientFactory&gt;().CreateClient(Name)</c>.
                /// Requires <see cref="Name"/> to be set.
                /// </summary>
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        RegisterEmptyInterceptorInterfaceDiagnostics(context);
        RegisterMissingRegistrationDiagnostics(context);
            ...
            .ForAttributeWithMetadataName(
                ...
                transform: static (ctx, _) =>
                {
                    ...
                    foreach (var attr in ctx.Attributes)
                    {
                        ...
                        // Collect all service types from this attribute (single or multi-type constructor)
                        var serviceTypesToCheck = new List<ITypeSymbol>();
                        if (attr.ConstructorArguments[0].Value is ITypeSymbol singleSt)
                            serviceTypesToCheck.Add(singleSt);
                        if (attr.ConstructorArguments.Length >= 2 && attr.ConstructorArguments[1].Value is ITypeSymbol st2)
                            serviceTypesToCheck.Add(st2);
                        if (attr.ConstructorArguments.Length >= 3 && attr.ConstructorArguments[2].Kind == TypedConstantKind.Array)
                            foreach (var item in attr.ConstructorArguments[2].Values)
                                if (item.Value is ITypeSymbol tx) serviceTypesToCheck.Add(tx);

                        foreach (var serviceType in serviceTypesToCheck)
                        {
    ...
    private static void RegisterDecoratorServiceTypeMismatchDiagnostics(
        IncrementalGeneratorInitializationContext context,
        string attributeFqn)
    {
        var mismatches = context.SyntaxProvider
            .ForAttributeWithMetadataName(
                ...
                transform: static (ctx, _) =>
                {
                    ...
                    foreach (var attr in ctx.Attributes)
                    {
                        ...
                        {
                            var loc = classSymbol.Locations.Length > 0 ? classSymbol.Locations[0] : Location.None;
                            return new DiagnosticInfo("AW012", loc,
                                new[] { classSymbol.Name, serviceType.ToDisplayString() });
                        }
                    }
    ...
    private static void RegisterEmptyInterceptorInterfaceDiagnostics(
        IncrementalGeneratorInitializationContext context)
    {
        var issues = context.SyntaxProvider
            .ForAttributeWithMetadataName(
                ...
                transform: static (ctx, _) =>
                {
                    if (ctx.TargetSymbol is not INamedTypeSymbol interceptorSymbol) return null;

                    foreach (var attr in ctx.Attributes)
                    {
                        if (attr.ConstructorArguments.Length == 0) continue;
                        if (attr.ConstructorArguments[0].Value is not INamedTypeSymbol serviceType) continue;

                        // Check if there are any interceptable methods
                        var hasInterceptable = false;
                        foreach (var member in serviceType.GetMembers().OfType<IMethodSymbol>())
                        {
                            ...
                            foreach (var p in member.Parameters)
                                if (p.RefKind != RefKind.None) { hasRefOut = true; break; }
                            if (!hasRefOut) { hasInterceptable = true; break; }
                        }

                        if (!hasInterceptable)
                        {
                            var loc = interceptorSymbol.Locations.Length > 0 ? interceptorSymbol.Locations[0] : Location.None;
                            return new DiagnosticInfo("AW011", loc, new[] { serviceType.Name, interceptorSymbol.Name });
                        }
                    }
                    return null;
                })
        ...
        context.RegisterSourceOutput(issues, static (ctx, d) =>
            ctx.ReportDiagnostic(d.Create(AW011EmptyInterceptorInterface)));
    }

    // ── AW013 helper ──────────────────────────────────────────────────────────

    private static void RegisterMissingRegistrationDiagnostics(
        IncrementalGeneratorInitializationContext context)
    {
        var missing = context.CompilationProvider
            .Select(static (compilation, _) => FindMissingRegistrations(compilation))
            .SelectMany(static (arr, _) => arr);

        context.RegisterSourceOutput(missing, static (ctx, d) =>
            ctx.ReportDiagnostic(d.Create(AW013MissingRegistration)));
    }

    private static ImmutableArray<DiagnosticInfo> FindMissingRegistrations(Compilation compilation)
    {
        ...
        var inspectableTypes = new HashSet<string>(StringComparer.Ordinal);

        foreach (var type in GetAllNamedTypesInNamespace(compilation.Assembly.GlobalNamespace))
        {
            if (type.IsAbstract) continue;

            var typeFqn = ToFullyQualified(type);
            foreach (var attr in type.GetAttributes())
            {
                ...
                {
                    case ScopedFqn:
                        AddIfNotNull(registrations, TransformSingle(type, attr, "Scoped", DuplicateStrategy.Add));
                        inspectableTypes.Add(typeFqn);
                        break;
                    case SingletonFqn:
                        AddIfNotNull(registrations, TransformSingle(type, attr, "Singleton", DuplicateStrategy.Add));
                        inspectableTypes.Add(typeFqn);
                        break;
                    case TransientFqn:
                        AddIfNotNull(registrations, TransformSingle(type, attr, "Transient", DuplicateStrategy.Add));
                        inspectableTypes.Add(typeFqn);
                        break;
                    case TryScopedFqn:
                        AddIfNotNull(registrations, TransformSingle(type, attr, "Scoped", DuplicateStrategy.Skip));
                        inspectableTypes.Add(typeFqn);
                        break;
                    case TrySingletonFqn:
                        AddIfNotNull(registrations, TransformSingle(type, attr, "Singleton", DuplicateStrategy.Skip));
                        inspectableTypes.Add(typeFqn);
                        break;
                    case TryTransientFqn:
                        AddIfNotNull(registrations, TransformSingle(type, attr, "Transient", DuplicateStrategy.Skip));
                        inspectableTypes.Add(typeFqn);
                        break;
                    case DecorateScopedFqn:
                        AddDecorator(decorators, type, attr, "Scoped");
                        inspectableTypes.Add(typeFqn);
                        break;
                    case DecorateSingletonFqn:
                        AddDecorator(decorators, type, attr, "Singleton");
                        inspectableTypes.Add(typeFqn);
                        break;
                    case DecorateTransientFqn:
                        AddDecorator(decorators, type, attr, "Transient");
                        inspectableTypes.Add(typeFqn);
                        break;
                    case HostedServiceFqn:
                        inspectableTypes.Add(typeFqn);
                        break;
                    case FactoryFqn:
                        AddFactoryRegistrations(registeredServiceTypes, inspectableTypes, type, attr);
                        break;
                    case HttpClientFqn:
                        AddHttpClientRegistrations(registeredServiceTypes, inspectableTypes, type, attr);
                        break;
                    case ValidateFqn:
                        AddValidatorRegistrations(registeredServiceTypes, inspectableTypes, type);
                        break;
                    case InterceptorFqn:
                        AddInterceptorRegistrations(registeredServiceTypes, inspectableTypes, type, attr);
                        break;
                }
            }
        }

        registrations.AddRange(FindScannedRegistrations(compilation));
        foreach (var reg in registrations)
        {
            inspectableTypes.Add(reg.ImplementationType);
            foreach (var svc in reg.ServiceTypes)
            {
                registeredServiceTypes.Add(svc);
                if (reg.IncludeLazy && !reg.IsOpenGeneric)
                    registeredServiceTypes.Add($"global::System.Lazy<{svc}>");
            }

            if (reg.IncludeSelf || reg.ServiceTypes.Contains(reg.ImplementationType))
                registeredServiceTypes.Add(reg.ImplementationType);
        }

        var serviceToConcreteMap = new Dictionary<string, string>(StringComparer.Ordinal);
        ...
        var decoratorGroups = new Dictionary<string, List<DecoratorInfo>>(StringComparer.Ordinal);
        foreach (var dec in decorators)
        {
            var key = dec.Lifetime + "::" + dec.ServiceType;
            if (!decoratorGroups.TryGetValue(key, out var list))
                decoratorGroups[key] = list = new List<DecoratorInfo>();
            list.Add(dec);
        }

        foreach (var kvp in decoratorGroups)
        {
            var chain = kvp.Value.OrderBy(static d => d.Order).ThenBy(static d => d.DecoratorType).ToList();
            if (!serviceToConcreteMap.TryGetValue(chain[0].ServiceType, out var originalConcrete))
                continue;

            registeredServiceTypes.Add(originalConcrete);
            for (var i = 0; i < chain.Count - 1; i++)
                registeredServiceTypes.Add(chain[i].DecoratorType);
        }
        ...
        {
            if (type.IsAbstract) continue;
            if (!inspectableTypes.Contains(ToFullyQualified(type))) continue;
            ...
            foreach (var ctor in type.InstanceConstructors)
            {
                ...
                foreach (var param in ctor.Parameters)
                {
                    if (IsFrameworkProvidedType(param.Type)) continue;
                    if (IsRegisteredWithAutoWireGraph(param.Type, registeredServiceTypes)) continue;

                    var loc = param.Locations.Length > 0
                        ? param.Locations[0]
                        : (type.Locations.Length > 0 ? type.Locations[0] : Location.None);

                    results.Add(new DiagnosticInfo("AW013", loc, new[]
                    {
                        type.Name,
                        param.Type.ToDisplayString(),
                        GetSuggestedImplementationName(param.Type)
                    }));
                }
            }
        }
    ...
    private static void AddIfNotNull(List<RegistrationInfo> registrations, RegistrationInfo? registration)
    {
        ...
        string lifetime)
    {
        ...
        if (attr.ConstructorArguments[0].Value is not ITypeSymbol serviceType) return;
        ...
        decorators.Add(new DecoratorInfo(
    ...
    private static void AddFactoryRegistrations(
        HashSet<string> registeredServiceTypes,
        HashSet<string> inspectableTypes,
        INamedTypeSymbol classSymbol,
        AttributeData attr)
    {
        if (attr.ConstructorArguments.Length == 0) return;
        if (attr.ConstructorArguments[0].Value is not ITypeSymbol productType) return;

        var factoryType = ToFullyQualified(classSymbol);
        inspectableTypes.Add(factoryType);
        registeredServiceTypes.Add(factoryType);
        registeredServiceTypes.Add(ToFullyQualified(productType));
    }

    private static void AddHttpClientRegistrations(
        HashSet<string> registeredServiceTypes,
        HashSet<string> inspectableTypes,
        INamedTypeSymbol classSymbol,
        AttributeData attr)
    {
        ...
        foreach (var na in attr.NamedArguments)
            if (na.Key == "UseFactory" && na.Value.Value is bool uf)
                useFactory = uf;

        if (useFactory) return;

        var implementationType = ToFullyQualified(classSymbol);
        inspectableTypes.Add(implementationType);
        registeredServiceTypes.Add(implementationType);
    }

    private static void AddValidatorRegistrations(
        HashSet<string> registeredServiceTypes,
        HashSet<string> inspectableTypes,
        INamedTypeSymbol classSymbol)
    {
        ...
        while (baseType is not null)
        {
            ...
            {
                inspectableTypes.Add(ToFullyQualified(classSymbol));
                registeredServiceTypes.Add($"global::FluentValidation.IValidator<{ToFullyQualified(baseType.TypeArguments[0])}>");
                return;
            }
    ...
    private static void AddInterceptorRegistrations(
        HashSet<string> registeredServiceTypes,
        HashSet<string> inspectableTypes,
        INamedTypeSymbol classSymbol,
        AttributeData attr)
    {
        ...
        var interceptorType = ToFullyQualified(classSymbol);
        inspectableTypes.Add(interceptorType);
        registeredServiceTypes.Add(interceptorType);
        registeredServiceTypes.Add(ToFullyQualified(serviceType));
    }

    private static bool IsRegisteredWithAutoWireGraph(
        ITypeSymbol type,
        HashSet<string> registeredServiceTypes)
    {
        var fqn = ToFullyQualified(type);
        if (registeredServiceTypes.Contains(fqn))
            return true;

        if (type is INamedTypeSymbol namedType && namedType.IsGenericType)
        {
            var openGenericName = GetOpenGenericName((INamedTypeSymbol)namedType.OriginalDefinition);
            if (registeredServiceTypes.Contains(openGenericName))
                return true;
        }
    ...
    private static bool IsFrameworkProvidedType(ITypeSymbol type)
    {
        var fqn = type.ToDisplayString();
        if (fqn is "System.IServiceProvider" or
            "Microsoft.Extensions.DependencyInjection.IServiceScopeFactory" or
            "Microsoft.Extensions.DependencyInjection.IServiceProviderIsService" or
            "Microsoft.Extensions.DependencyInjection.IKeyedServiceProvider" or
            "Microsoft.Extensions.Configuration.IConfiguration" or
            "Microsoft.Extensions.Hosting.IHostEnvironment" or
            "Microsoft.AspNetCore.Hosting.IWebHostEnvironment" or
            "System.Net.Http.HttpClient" or
            "Microsoft.Extensions.Http.IHttpClientFactory" or
        ...
        if (type is INamedTypeSymbol namedType && namedType.IsGenericType)
        {
            ...
            if (original is "Microsoft.Extensions.Logging.ILogger<TCategoryName>" or
                "Microsoft.Extensions.Options.IOptions<TOptions>" or
                "Microsoft.Extensions.Options.IOptionsSnapshot<TOptions>" or
                "Microsoft.Extensions.Options.IOptionsMonitor<TOptions>" or
                "Microsoft.Extensions.Options.IOptionsFactory<TOptions>" or
                "Microsoft.Extensions.Options.IOptionsMonitorCache<TOptions>" or
                "Microsoft.Extensions.Options.IOptionsChangeTokenSource<TOptions>" or
                "Microsoft.Extensions.Options.IConfigureOptions<TOptions>" or
                "Microsoft.Extensions.Options.IPostConfigureOptions<TOptions>" or
                "Microsoft.Extensions.Options.IValidateOptions<TOptions>" or
    ...
    private static string GetSuggestedImplementationName(ITypeSymbol type)
    {
        var name = type.Name;
    ...
    private static bool HasAnyRegistrationAttribute(INamedTypeSymbol type)
    {
        foreach (var attr in type.GetAttributes())
        {
            ...
            if (fqn == ScopedFqn || fqn == SingletonFqn || fqn == TransientFqn ||
                ...
                fqn == DecorateScopedFqn || fqn == DecorateSingletonFqn || fqn == DecorateTransientFqn ||
            ...
            .ForAttributeWithMetadataName(
                ...
                transform: static (ctx, _) =>
                {
                    ...
                    foreach (var na in attr.NamedArguments)
                    {
                        ...
                        else if (na.Key == "UseFactory" && na.Value.Value is bool uf)
                            useFactory = uf;
                    }

                    return new HttpClientInfo(ToFullyQualified(classSymbol), name, baseAddress, resilience, timeoutSeconds, headersBuilder.ToImmutable(), useFactory);
                })
    ...
    private static RegistrationInfo? TransformSingle(
        ...
        DuplicateStrategy defaultStrategy)
    {
        // ── Read explicit service type(s) from constructor args ───────────────
        // Single-type: ctor(Type serviceType) → ConstructorArguments[0].Kind = Type
        // Multi-type:  ctor(Type t1, Type t2, params Type[] rest) →
        //   ConstructorArguments[0] = t1, [1] = t2, [2] = array (rest)
        ITypeSymbol? explicitServiceType = null;
        var extraServiceTypes = new List<ITypeSymbol>();

        if (attr.ConstructorArguments.Length == 1 && attr.ConstructorArguments[0].Value is ITypeSymbol st0)
        {
            explicitServiceType = st0;
        }
        else if (attr.ConstructorArguments.Length >= 2)
        {
            // Multi-type constructor
            if (attr.ConstructorArguments[0].Value is ITypeSymbol t1) { explicitServiceType = t1; extraServiceTypes.Add(t1); }
            if (attr.ConstructorArguments[1].Value is ITypeSymbol t2) extraServiceTypes.Add(t2);
            // Third arg is params Type[] rest
            if (attr.ConstructorArguments.Length >= 3 &&
                attr.ConstructorArguments[2].Kind == TypedConstantKind.Array)
            {
                foreach (var item in attr.ConstructorArguments[2].Values)
                    if (item.Value is ITypeSymbol tx) extraServiceTypes.Add(tx);
            }
        }
        ...
        if (extraServiceTypes.Count > 0)
        {
            // Multi-type constructor: register against all specified types
            foreach (var st in extraServiceTypes)
                closedServiceTypes.Add(ToFullyQualified(st));
        }
        else if (explicitServiceType is not null)
        {
        ...
        string methodName)
    {
        ...
        if (!httpClients.IsEmpty)
        {
            ...
            foreach (var hc in httpClients.OrderBy(static h => h.ImplementationType))
            {
                ...
                // UseFactory mode: register named client + IHttpClientFactory factory delegate
                if (hc.UseFactory && hc.Name is not null)
                {
                    var factoryLine = $"        services.AddHttpClient(\"{hc.Name}\")";
                    if (hc.Resilience)
                        factoryLine += "\n            .AddStandardResilienceHandler()";
                    sb.AppendLine(factoryLine + ";");
                    sb.AppendLine($"        services.AddScoped(sp =>");
                    sb.AppendLine($"            sp.GetRequiredService<global::Microsoft.Extensions.Http.IHttpClientFactory>().CreateClient(\"{hc.Name}\"));");
                    continue;
                }
