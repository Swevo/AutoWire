...
### Added
- **Code fix for AW007** — "Register as self: [Scoped(typeof(ClassName))]": adds an explicit `typeof(ClassName)` constructor argument to the registration attribute, making the self-registration intent clear and suppressing the AW007 info diagnostic.
...
### Added
- **Code fix for AW010** — "Remove duplicate [Interceptor]": removes the duplicate `[Interceptor]` attribute when two attributes on the same class target the same interface.
